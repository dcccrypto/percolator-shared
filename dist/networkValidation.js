...
export function validateNetworkConfig(env) {
    ...
    const network = (env.NETWORK?.toLowerCase() || "").trim();
    if (!["devnet", "testnet", "mainnet"].includes(network)) {
        throw new Error(`❌ NETWORK env var must be set to 'devnet', 'testnet', or 'mainnet'.\n` +
    ...
    if (network === "mainnet" && !env.FORCE_MAINNET) {
        throw new Error(`⛔ MAINNET SAFETY GUARD ACTIVE\n\n` +
    ...
    if (!rpcUrl) {
        if (network === "mainnet") {
            throw new Error(`❌ RPC_URL env var MUST be set for mainnet operations.\n` +
    ...
    if (!programIdEnv) {
        throw new Error(`❌ PROGRAM_ID env var MUST be set.\n` +
    ...
    if (!/^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(programIdEnv)) {
        throw new Error(`❌ PROGRAM_ID does not look like a valid Solana address.\n` +
            `Got: ${programIdEnv}\n` +
 ...
 */
export function ensureNetworkConfigValid(env) {
    try {
