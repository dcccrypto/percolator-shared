...
export async function withRetry(fn, opts = {}) {
    const { maxRetries = 3, baseDelayMs = 1000, maxDelayMs = 30000, rateLimit429MinDelayMs = 1000, rateLimit429MaxDelayMs = 30000, label = "operation", } = opts;
    let lastError;
        ...
        try {
            ...
            if (is429Error(error)) {
                ...
                if (retryAfterMs !== null) {
                    ...
                    console.warn(`[Retry] ${label} rate-limited (429) on attempt ${attempt + 1}/${maxRetries + 1}: ${errorMessage}. ` +
                    ...
                    delay = Math.min(Math.max(exponentialDelay + jitter, rateLimit429MinDelayMs), rateLimit429MaxDelayMs);
                    console.warn(`[Retry] ${label} rate-limited (429) on attempt ${attempt + 1}/${maxRetries + 1}: ${errorMessage}. ` +
