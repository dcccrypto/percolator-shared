 ...
 */
export function loadSealedKeypair(env) {
    const rawKey = env.CRANK_KEYPAIR;
    // 1. Validate key is set
    if (!rawKey) {
        throw new Error("❌ CRANK_KEYPAIR env var is required.\n" +
    ...
    try {
        ...
        try {
            ...
            throw new Error("❌ Invalid CRANK_KEYPAIR format.\n" +
                "Must be either:\n" +
 ...
 */
function createSealedSigner(keypair, auditEnabled) {
 ...
 */
export function validateSigner(signer, expectedPublicKey) {
    const publicKey = signer.publicKey();
    // Basic validation: public key looks like a Solana address
    if (!/^[1-9A-HJ-NP-Za-km-z]{32,44}$/.test(publicKey)) {
        throw new Error(`❌ Invalid public key from signer: ${publicKey}. ` +
